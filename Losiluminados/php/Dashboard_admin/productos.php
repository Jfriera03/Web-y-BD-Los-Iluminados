<?php
    ...
    //Consultas para obtener los productos clasificados por tipo con detalles adicionales
    $sql_vm = "
        SELECT 
            p.idProducto, 
            p.nombreProducto, 
            ...
            e.nombreEtapa, 
            p.idVM,
            vm.precioBase + c.precio + r.precio + a.precio AS precioTotal
        FROM PRODUCTO p
    ...
    $sql_bd = "
        SELECT 
            ...
            bd.numConexiones, 
            p.idBD,
            bd.precioBase + s.precio AS precioTotal
        FROM PRODUCTO p
        JOIN VLAN v ON p.idVLAN = v.idVLAN
    ...
    $sql_ac = "
        SELECT 
            ...
            p.idAC,
            e.nombreEtapa,
            ac.precioBase + SUM(cu.precio) AS precioTotal
        FROM PRODUCTO p
        JOIN VLAN v ON p.idVLAN = v.idVLAN
                    ...
                    <?php if ($result_vm->num_rows > 0): ?>
                        <?php while ($row = $result_vm->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="/php/Dashboard_admin/editar_productoVM.php?idVM=<?= htmlspecialchars($row['idVM'], ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-warning <?= ($tienePrivilegioEditar == '0') ? 'disabled' : '' ?>">Editar</a>
                                    <a href="?delete_vm=<?= $row['idVM'] ?>" class="btn btn-sm btn-danger <?= ($tienePrivilegioEliminar == '0') ? 'disabled' : '' ?>" onclick="return confirm('¿Estás seguro de que quieres eliminar este producto?')">Eliminar</a>
                                </td>
                    ...
                    <?php if ($result_bd->num_rows > 0): ?>
                        <?php while ($row = $result_bd->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($row['nombreEtapa']) ?></td>
                                <td>
                                    <a href="/php/Dashboard_admin/editar_productoBD.php?idBD=<?= htmlspecialchars($row['idBD'], ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-warning <?= ($tienePrivilegioEditar == '0') ? 'disabled' : '' ?>">Editar</a>
                                    <a href="?delete_bd=<?= $row['idBD'] ?>" class="btn btn-sm btn-danger <?= ($tienePrivilegioEliminar == '0') ? 'disabled' : '' ?>" onclick="return confirm('¿Estás seguro de que quieres eliminar este producto?')">Eliminar</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                    ...
                    <?php if ($result_ed->num_rows > 0): ?>
                        <?php while ($row = $result_ed->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($row['nombreEtapa']) ?></td>
                                <td>
                                    <a href="/php/Dashboard_admin/editar_productoED.php?idED=<?= htmlspecialchars($row['idED'], ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-warning <?= ($tienePrivilegioEditar == '0') ? 'disabled' : '' ?>" >Editar</a>
                                    <a href="?delete_ed=<?= $row['idED'] ?>" class="btn btn-sm btn-danger <?= ($tienePrivilegioEliminar == '0') ? 'disabled' : '' ?>" onclick="return confirm('¿Estás seguro de que quieres eliminar este producto?')">Eliminar</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                    ...
                    <?php if ($result_ac->num_rows > 0): ?>
                        <?php while ($row = $result_ac->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($row['nombreEtapa']) ?></td>
                                <td>
                                    <a href="/php/Dashboard_admin/editar_productoAC.php?idAC=<?= htmlspecialchars($row['idAC'], ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-warning <?= ($tienePrivilegioEditar == '0') ? 'disabled' : '' ?>">Editar</a>
                                    <a href="?delete_ac=<?= $row['idAC'] ?>" class="btn btn-sm btn-danger <?= ($tienePrivilegioEliminar == '0') ? 'disabled' : '' ?>" onclick="return confirm('¿Estás seguro de que quieres eliminar este producto?')">Eliminar</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
