...
$sql = "
    ...
    JOIN producto p ON ed.idED = p.idED
    WHERE ed.idED = ? 
";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->execute()) {
        ...
        $stmt_etapa->bind_param("ii", $idEtapa, $idED);
        $stmt_etapa->execute();

        echo "<script>alert('Entorno de Desarrollo actualizado correctamente.'); window.location.href = '/php/Dashboard_admin/productos.php';</script>";
    } else {
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="etapa" name="etapa" required>
                <?php foreach ($etapas as $etapa): ?>
                    <option value="<?= $etapa['idEtapa'] ?>" <?= $etapa['idEtapa'] == $entorno['idEtapa'] ? 'selected' : '' ?>>
