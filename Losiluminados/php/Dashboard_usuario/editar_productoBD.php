...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$numConexiones || $recuperarDatos === null || $controlAcceso === null || !$idSGBD || !$capacidadSeleccionada) {
    ...
    if ($stmt1->execute() && $stmt2->execute()) {
        if ($stmt1->affected_rows > 0 || $stmt2->affected_rows > 0) {
            echo "<script>alert('Base de Datos actualizada correctamente.'); window.location.href = '/php/Dashboard_usuario/productos.php';</script>";
        } else {
            echo "<script>alert('No se realizaron cambios en la Base de Datos.'); window.location.href = '/php/Dashboard_usuario/productos.php';</script>";
        }
    } else {
