...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$idVM || !$idCPU || !$idRAM || !$idAlmacenamiento || !$idSO) {
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            echo "<script>alert('Máquina Virtual actualizada correctamente.'); window.location.href = '/php/Dashboard_usuario/productos.php';</script>";
        } else {
            echo "<script>alert('No se realizaron cambios en la Máquina Virtual.'); window.location.href = '/php/Dashboard_usuario/productos.php';</script>";
        }
    } else {
